...
void readCounterFromFlash(const char *type, uint32_t &counter); // get the rolling code counter from setup.json & return it
void writeCounterToFlash(const char *type, uint32_t &counter); // write the counter back to setup.json
